...
public class HugeFungusHandler implements ITreeBlock {
    private static final Lazy<Set<Block>> logs = new Lazy<>(() -> ModConfig.getIdentifiedBlocks(ModConfig.get().hugeFungusLogs).collect(Collectors.toSet()));
    private static final Lazy<Set<Block>> leaves = new Lazy<>(() -> ModConfig.getIdentifiedBlocks(ModConfig.get().hugeFungusLeaves).collect(Collectors.toSet()));
